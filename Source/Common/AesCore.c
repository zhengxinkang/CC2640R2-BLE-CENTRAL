...
{
	unsigned char i;
	unsigned char p1[32]="191345678x1235f23w3s4r4343423455";
	unsigned char p2[48]="291345678x1235f23w3s4r4343423456";
	unsigned char pKey[] = "TTMJ_123TTMJ_123";
	unsigned char pIV[48] = "mplock1234567890";
	unsigned char pPlainText[64];
	unsigned char pPlainText1[64];
	unsigned char pPlainText2[64];

	unsigned char pCipherText[64];
	unsigned char pCipherText1[64];
	unsigned char pCipherText2[64];

	AES_Init(pKey);
	memset(pCipherText,0,sizeof(pCipherText));
	memset(pPlainText,0,sizeof(pCipherText));
	AES_Encrypt(p1, pCipherText,32, pIV);//加密
	AES_Decrypt(pPlainText,pCipherText,32,pIV);//解密

	memset(pCipherText1,0,sizeof(pCipherText));
	memset(pPlainText1,0,sizeof(pCipherText));
	AES_Encrypt(p2, pCipherText1,32, pIV);//加密
	AES_Decrypt(pPlainText1,pCipherText1,32,pIV);//解密

	memset(pCipherText2,0,sizeof(pCipherText));
	memset(pPlainText2,0,sizeof(pCipherText));
	memcpy(pCipherText2,pCipherText1,16);
	memcpy((unsigned char*)&pCipherText2[16],(unsigned char*)&pCipherText[16],16);
	AES_Decrypt(pPlainText2,pCipherText2,32,pIV);//解密

	memset(pCipherText,0,sizeof(pCipherText));
	memset(pPlainText,0,sizeof(pCipherText));
	AES_Encrypt(p2, pCipherText,48, pIV);//加密
	AES_Decrypt(pPlainText,pCipherText,48,pIV);//解密
	for(i=0;i<32;i++)
	{
		p2[i]=i;
	}
